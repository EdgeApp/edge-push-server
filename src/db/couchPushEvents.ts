 ...
 * Mutate the `event` object, then call `save` to commit the changes.
 */
...
type CouchPushEvent = ReturnType<typeof asCouchPushEvent>
...
export async function getEventsByDeviceId(
  ...
  return response.rows.map(row => makePushEventRow(connection, row.doc))
}
...
export async function* streamAddressBalanceEvents(
  ...
  for await (const raw of stream) {
    yield makePushEventRow(connection, raw)
  }
}
...
function makePushEventRow(connection: ServerScope, raw: unknown): PushEventRow {
  const db = connection.db.use(couchEventsSetup.name)
  const clean = asCouchPushEvent(raw)
  let { id, rev } = clean
  const event = clean.doc
  let base = { ...event }
  ...
  return {
    ...
    async save(): Promise<void> {
      while (true) {
        ...
        const doc: CouchPushEvent = { doc: event, id, rev }
        ...
        if (event.broadcastTxErrors === base.broadcastTxErrors) {
          event.broadcastTxErrors = remote.broadcastTxErrors
        }
        if (event.pushMessageError === base.pushMessageError) {
          event.pushMessageError = remote.pushMessageError
        }
        if (event.state === base.state) {
          event.state = remote.state
        }
        if (event.triggered === base.triggered) {
          event.triggered = remote.triggered
        }
      }
    }
  }
